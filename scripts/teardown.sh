...
# Delete the caeritificate we created. 
cn=$(cd ${ROOT}/terraform 1>&2 > /dev/null ;  terraform refresh  -var-file=terraform.tfvars 1>&2 > /dev/null && terraform output cluster_name)
if [ -z "$cn" ]; then 
   echo "ERROR: Could not get the cluste rname"
   exit 
fi 
echo "Deleting Managed Certificate $cn"
gcloud compute ssl-certificates delete $cn --quiet
